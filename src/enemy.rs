use bevy::prelude::*;
use crate::animations::{AnimationController, AnimationData, CharacterAnimations, CharacterState, CurrentAnimation};
...
fn update_enemy_movement(
    mut enemies: Query<(&mut Enemy, &mut Transform, &mut Physics, &mut AnimationController)>,
    ...
    for (mut enemy, mut transform, mut physics, mut animation_controller) in &mut enemies {
        ...
        if abs_distance < enemy.detection_range {
            ...
            if abs_distance < enemy.attack_range {
                ...
                physics.velocity.x = if distance > 0.0 { enemy.speed } else { -enemy.speed };
                if animation_controller.get_current_state() != CharacterState::Attacking 
                   && animation_controller.get_current_state() != CharacterState::Hurt {
...
fn update_enemy_animations(
    mut enemies: Query<(&mut AnimationController, &Physics, &Enemy)>,
) {
...
fn handle_damage(
    ...
    for (mut enemy, enemy_transform, mut animation_controller) in &mut enemies {
        ...
        for (hitbox, hitbox_transform) in &hitboxes {
            if hitbox.active {
                let distance = (hitbox_transform.translation - enemy_transform.translation).length();
...
fn check_death(
    mut query: Query<(&mut Enemy, &mut AnimationController)>,
) {
